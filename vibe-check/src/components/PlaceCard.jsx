...
// ------------------------------------------------------------------
// The individual card component used in the Grid View.
// Props:
// - place: The place data object from Google API or local storage
// - onClick: Handler to open the modal
// - onToggleFavorite: Handler to add/remove from favorites
// - isFavorite: Boolean to switch the heart icon state
...
const PlaceCard = ({ place, onClick, onToggleFavorite, isFavorite }) => {
  ...
  // 1. SMART IMAGE LOGIC
  // ------------------------------------------------------------------
  // Google Places API returns photos as functions (getUrl()).
  // Saved Bookmarks store photos as strings (storedPhotoUrl).
  // We check these in order, falling back to a placeholder if none exist.
  ...
  // 2. DATA SAFEGUARDS
  // ------------------------------------------------------------------
  // Ensure we have default values for Vibe data if it's missing (e.g. loading error)
  ...
  return (
    ...
    >
      ...
      <div className="p-8">
        ...
        {/* Place Name */}
        <h3 className="text-2xl font-black tracking-tight text-slate-900 group-hover:text-indigo-600 transition-colors truncate mb-4">
            {place.name}
        </h3>
        
        {/* Footer Info: Address & Open Status */}
